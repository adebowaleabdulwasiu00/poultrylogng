...
export default function PurchaseLineItemRow({ item, index, products, onChange, onRemove, canRemove, currencySymbol }: Props) {
                ...
                {filtered.length === 0 ? (
                  ...
                  filtered.slice(0, 10).map(p => (
                    <div key={p.id} className="customer-dropdown-item" onClick={() => selectProduct(p)}>
                      ...
                      <div className="text-sm text-muted">Stock: {p.currentStock} {p.unit} | Cost: {currencySymbol}{p.purchasePrice.toLocaleString()}</div>
                    </div>
                  ))
                )}
              </div>
