...
import type { SalesHeader, SalesDetail } from '@/db/types';
...
export default function PrintableInvoice({ header, details, companyInfo }: Props) {
  ...
  function handlePrint() {
    ...
    if (!win) return;
    win.document.write(`
      <html><head><title>Invoice ${header.invoiceNumber}</title>
      <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        body { font-family: 'Inter', Arial, sans-serif; font-size: 12px; color: #1C1B1F; padding: 20px; }
        .invoice-header { display: flex; justify-content: space-between; margin-bottom: 24px; border-bottom: 2px solid #2E7D32; padding-bottom: 16px; }
        .company-info h2 { font-size: 18px; color: #2E7D32; margin-bottom: 4px; }
        .company-info p { font-size: 11px; color: #666; line-height: 1.6; }
        .invoice-title { text-align: right; }
        .invoice-title h1 { font-size: 28px; color: #2E7D32; letter-spacing: 2px; }
        ...
        .items-table th { background: #2E7D32; color: #fff; padding: 8px 12px; text-align: left; font-size: 11px; font-weight: 600; }
        ...
        .totals-line.total { border-top: 2px solid #2E7D32; padding-top: 8px; margin-top: 4px; font-weight: 700; font-size: 14px; color: #2E7D32; }
  ...
  const discountAmount = subtotal * (header.discountPercent / 100);
      ...
      <div ref={printRef} className="printable-invoice">
        <div className="invoice-header">
          ...
          <div className="invoice-title">
            <h1>INVOICE</h1>
            <p><strong>{header.invoiceNumber}</strong></p>
            <p>Date: {header.invoiceDate}</p>
        ...
        <div className="invoice-meta">
          ...
          <div className="meta-box">
            <h4>Status</h4>
            <p style={{ textTransform: 'uppercase', fontWeight: 700, color: header.status === 'posted' ? '#2E7D32' : header.status === 'cancelled' ? '#D32F2F' : '#F57F17' }}>{header.status}</p>
          </div>
        </div>
        ...
        <table className="items-table">
          ...
          <tbody>
            {details.map((d, i) => (
              <tr key={d.id}>
                <td>{i + 1}</td>
                <td><strong>{d.productName}</strong>{d.description && <><br /><small style={{ color: '#999' }}>{d.description}</small></>}</td>
                <td>{d.quantity}</td>
                <td>{d.unit}</td>
        ...
        <div className="totals">
          <div className="totals-box">
            <div className="totals-line">
              <span>Subtotal</span>
              <span>{companyInfo.currencySymbol}{subtotal.toLocaleString()}</span>
            </div>
            {header.discountAmount > 0 && (
              <div className="totals-line">
                <span>Discount ({header.discountPercent}%)</span>
                <span>-{companyInfo.currencySymbol}{discountAmount.toLocaleString()}</span>
              </div>
            )}
