...
import type { CustomerPayment } from '@/db/types';
...
export default function CustomerPayments() {
  ...
  const { paymentMethods, allBanks } = useSettings();
  const paymentMethodNames = paymentMethods.length > 0 ? paymentMethods.map((m: { methodName: string }) => m.methodName) : PAYMENT_METHODS;
  ...
  function openNew() {
    ...
    setForm({
      paymentDate: new Date().toISOString().split('T')[0],
      customerId: 0, amountPaid: 0, paymentMethod: 'Cash', paymentReference: '',
      invoiceNumber: '', bank: '', cashier: '', receiptNumber: '', notes: '', attachment: '',
    });
    setShowForm(true);
  }
  ...
  async function handleSubmit() {
    ...
    if (!customer) { toast('Invalid customer', 'error'); return; }
    const now = nowISO();
    ...
    const record: Omit<CustomerPayment, 'id'> = {
      ...
      status: 'completed',
      receiptNumber: form.receiptNumber || `REC-${Date.now().toString(36).toUpperCase()}`,
      notes: form.notes,
    ...
    if (editing?.id) {
      await db.customerPayments.update(editing.id, { ...record, syncStatus: 'pending' });
      await queueSync('customerPayments', editing.id, 'update', record);
      toast('Payment updated');
    } else {
      const id = await db.customerPayments.add(record as CustomerPayment);
      await db.customers.update(form.customerId, { lastPaymentDate: form.paymentDate, updatedAt: now, syncStatus: 'pending' });
  ...
  async function handleDelete() {
    ...
    await db.customerPayments.delete(deleteTarget.id);
    await queueSync('customerPayments', deleteTarget.id, 'delete', null);
    ...
    <div>
      <div className="grid grid-4 mb-6">
        <div className="stat-card">
          ...
          <div className="stat-info"><h3>₦{stats.total.toLocaleString()}</h3><p>Total Collections</p></div>
        </div>
        <div className="stat-card">
          <div className="stat-icon blue"><span className="material-icons-outlined">today</span></div>
          <div className="stat-info"><h3>₦{stats.todayTotal.toLocaleString()}</h3><p>Today's Collections</p></div>
        </div>
        <div className="stat-card">
          <div className="stat-icon purple"><span className="material-icons-outlined">receipt</span></div>
          <div className="stat-info"><h3>{stats.count}</h3><p>Total Payments</p></div>
        </div>
        <div className="stat-card">
      ...
      {filtered.length === 0 ? (
        <EmptyState
          icon="payments"
          title="No Payments"
          description="Record customer payments to track collections"
              ...
              <tbody>
                {filtered.map(p => (
                  <tr key={p.id}>
                    ...
                    <td>
                      <button className="btn btn-text btn-sm" onClick={() => navigate(`/customers/${p.customerId}`)} style={{ padding: 0, fontWeight: 600 }}>
                        {p.customerName}
                      </button>
                    </td>
                    ...
                    <td className="text-sm">{p.cashier || '-'}</td>
                    <td>
                      {p.attachment ? (
      ...
      >
        <div className="form-row cols-2">
          ...
          <div className="form-group">
            ...
            <select className="form-select" value={form.customerId || ''} onChange={e => setForm(f => ({ ...f, customerId: Number(e.target.value) }))}>
        ...
        <div className="form-row cols-2">
          <div className="form-group">
            ...
            <input className="form-input" value={form.invoiceNumber} onChange={e => setForm(f => ({ ...f, invoiceNumber: e.target.value }))} placeholder="Optional" />
          </div>
          <div className="form-group">
        ...
        <div className="form-group">
          ...
          <input className="form-input" value={form.cashier} onChange={e => setForm(f => ({ ...f, cashier: e.target.value }))} placeholder="Staff name" />
        </div>
        <div className="form-group">
      ...
      <ConfirmDialog
        ...
        message={`Delete payment of ₦${deleteTarget?.amountPaid.toLocaleString()} from "${deleteTarget?.customerName}"?`}
